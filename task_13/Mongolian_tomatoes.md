...
graph LR
    C(C - 0 фр.)  --> 3(3 - 1 фр.) -.-> B(B - 2 фр.) --> 4(4 - 4 фр.) -.-> A(A - 5 фр.) --> 1(1 - 6 фр.)
    A(A - 5 фр.) --> 2(2 - 6 фр.)
    A(A - 5 фр.) --> 5(5 - 6 фр.) -.-> D(D - 7 фр.)
```
В построенном дереве нет цепей, чередующееся относительно текущего паросочетания, ветка закончилась в покрытых вершинах, то есть в указанном графе нет совершенного паросочетания.

5) Проведем еще одну редукцию матрицы затрат.
...
Необходимо найти минимальный элемент из строк, включенных во множество X и столбцов, не включенных во множество Y. В нашем случае это  строки A,B,C,D и столбец 1. Минимальный элемент 0, расположен в строке A и столбце 1. 
