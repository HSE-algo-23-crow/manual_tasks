...
graph LR
    B(B - 0 фр.)  --> 1(1 - 1 фр.) -.-> E(E - 2 фр.)
    B(B - 0 фр.)  --> 4(4 - 1 фр.) -.-> A(A - 2 фр.) --> 5(5 - 3 фр.) 
    A(A - 2 фр.)  --> 2(2 - 3 фр.) -.-> D(D - 4 фр.)
```
Построенное дерево содержит чередующуюся, относительно текущего паросочетания, цепь B4 - 4A - A5, цепь начинается и заканчивается в непокрытых вершинах, все ребра в цепи чередуются по вхождению в текущее паросочетание.
```mermaid
graph LR
    B --> 4 -.-> A --> 5
```
